...
def is_valid_tour(tour):
    """ Checks if the generated tour is valid based on travel_data """
    for i in range(len(tour) - 1):
        if not ((travel_data['Source'] == tour[i]) & (travel_data['Destination'] == tour[i + 1])).any():
            return False
    return True
...
def calculate_fitness(tour, max_time, max_cost):
    ...
    for i in range(len(tour) - 1):
        row = merged_data[
            (merged_data["Source"] == tour[i]) & (merged_data["Destination"] == tour[i + 1])
        ]
        if not row.empty:
...
def crossover(parent1, parent2):
    ...
    return child if is_valid_tour(child) else parent1
...
def mutate(tour, mutation_rate=0.1):
    ...
    return tour if is_valid_tour(tour) else mutate(tour, mutation_rate)
...
optimal_tour, total_time, total_cost, average_rating = genetic_algorithm(starting_city, max_time, max_cost)
if optimal_tour:
