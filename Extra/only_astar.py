...
if __name__ == "__main__":
    ...
    if source and destination and time_limit is not None and budget is not None:
        ...
        if source not in city_to_vertex or destination not in city_to_vertex:
            ...
            if path:
                # print("new : ", city_to_vertex)
                l = []
                for node in path:
                    l.append(node.city)
                    print(node.time, node.cost)
                tt, tc = 0, 0
                # print(l)
                ...
                print(
                    f"No path found from {source} to {destination} within the specified time limit and budget."
                )
    else:
