...
class AntColony(object):
    ...
    def pick_move(self,pheromone, time, cost, visited):
        ...
        # if np.any(time == 0) or np.any(time == 0):
        #     print("yes zero")
        if row.sum() == 0:
            return None
...
#reading places
    ...
    api_path = api_sol[0]
    print("A* path: ")
    for i,place in enumerate(api_path):
        print(i+1,place)
    print("time taken: ",api_sol[1])
    print("cost taken: ",api_sol[2])
    N = len(api_path)
    avg_R = api_sol[3]
    R = avg_R*N
    budget = budget - api_sol[2]
    time_limit = time_limit - api_sol[1]
