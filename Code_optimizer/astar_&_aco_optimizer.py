...
class AntColony(object):
    ...
    def __init__(self, times,costs, n_ants, n_best, n_iterations, decay, alpha=1, beta=1):
        self.times  = times
        self.costs = costs
    ...
    def gen_path(self, start):
        ...
        for i in range(len(self.times) - 1):
            move = self.pick_move(self.pheromone[prev],self.times[prev],self.costs[prev],visited)
            if not move:
                break
...
#reading places
    ...
    api_source = input("where do you live? ")
    api_destination = city
    
    with open('analysis.csv',mode = 'w',newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["iteration","ants","execution_time","total_time","total_cost","avg_rating","path"])
        for i in range(200):
            ITER += 5
            ANTS += 5
            api_sol = sofia_astar.API(api_source,api_destination,time_limit,budget)
            if not api_sol:
            ...
            start = time.time()
            # times,costs, n_ants, n_best, n_iterations, decay, alpha=1, beta=1
            ant_colony = AntColony(times,costs,ANTS,10,ITER,1.5)
            ...
            for place in best_path[0]:
                ...
                num_places += 1
            mypath.pop()
            total_cost += api_sol[2]
            total_time += api_sol[1]
            pathstring = ' '.join(mypath)
            avg_rating = (R + total_rating)/(num_places + N)
            myrow = []
            myrow.append(ITER)
            myrow.append(ANTS)
            myrow.append(end-start)
            myrow.append(total_time)
            myrow.append(total_cost)
            myrow.append(avg_rating)
            myrow.append(pathstring)
