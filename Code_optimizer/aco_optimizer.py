...
if __name__ == "__main__":
    ...
    with open("analysis3.csv", mode="w", newline="") as file:
        ...
        for i in range(200):
            ...
            with open("travel.csv", newline="") as travel:
                ...
                next(reader)
                for row in reader:
                    times[name_num[row[0].strip()]][name_num[row[1].strip()]] = (
                        float(row[2]) * 100
                    )
                    times[name_num[row[1].strip()]][name_num[row[0].strip()]] = (
                        float(row[2]) * 100
                    )
                    costs[name_num[row[0].strip()]][name_num[row[1].strip()]] = float(
                        row[3]
                    )
                    costs[name_num[row[1].strip()]][name_num[row[0].strip()]] = float(
                        row[3]
                    )
            start = time.time()
            ...
            total_time, total_cost, total_rating = (
                start_info[2],
                start_info[3],
                start_info[1],
            )
            ...
            myrow.append(total_rating / num_places)
