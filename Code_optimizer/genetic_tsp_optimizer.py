...
class GeneticAlgorithm:
    ...
    def show_customization_dialog(self, root, starting_city):
        ...
        for place in city_places:
            var = IntVar()
            chk = Checkbutton(
                dialog, text=place, variable=var, anchor="w", width=40, pady=5
            )
            chk.pack()
            vars[place] = var
        ...
        def confirm():
            self.places_customization = [
                place for place, var in vars.items() if var.get() == 1
            ]
            dialog.destroy()
            root.destroy()
    ...
    def gui_input(self):
        ...
        source_astar = simpledialog.askstring(
            "Input", "Where do you want to start your journey: ", parent=root
        )
        while True:
            value = simpledialog.askstring(
                "Input", "Enter 1 for all places and 2 for customization: ", parent=root
            )
            if value not in ["1", "2"]:
        ...
        if value == "1":
            places = list(self.places_data.index)
            self.places_customization = [
                p for p in places if self.extract_city_name(p) == starting_city
            ]
            root.destroy()
        elif value == "2":
    ...
    def run(
        ...
        city_places = [
            p
            for p in self.places_customization
            if self.extract_city_name(p) == starting_city
        ]
        if not city_places:
        ...
        best_tour, best_time, best_rating, generation_no, best_cost = (
            None,
            0,
            0,
            0,
        ...
        for i in range(generations):
            ...
            if ((best_time <= time) and (time <= max_time)) and (
                (cost <= best_cost) and (cost <= max_cost)
            ):
                best_time = time
                best_cost = cost
                best_rating = rating
...
if __name__ == "__main__":
    ...
    if not starting_city:
        ...
        exit()
    with open("analysis2.csv", mode="w", newline="") as file:

        writer = csv.writer(file)
        # writer.writerow(["population","generation","execution_time","total_time","total_cost","avg_rating","path"])
        writer.writerow(["generations", "population", "total_time"])
        file.flush()
        population_size, generations = 10, 10
        for i in range(20):
            curr_max_time = max_time
            curr_max_cost = max_cost
            population_size += 10
            # generations += 10
            start_time = time.time()
            api_sol = sofia_astar.API(
                source_astar,
            ...
            curr_max_cost = curr_max_cost - api_sol[2]
            curr_max_time = curr_max_time - api_sol[1]
            optimal_tour, total_time, total_cost, average_rating, generation_no = (
                optimizer.run(
                    starting_city,
                    curr_max_time,
                    curr_max_cost,
            ...
            # row.append((total_time*average_rating)/total_cost)
